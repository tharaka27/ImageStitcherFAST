...
namespace TORBHLS {
	...
	void ComputePyramid(cv::Mat image, float* mvInvScaleFactor, cv::Mat* mvImagePyramid) {
		...
		for (int level = 0; level < _nlevels; ++level) {
			...
			if (level != 0)
			{
				...
				copyMakeBorder(mvImagePyramid[level], temp, EDGE_THRESHOLD, EDGE_THRESHOLD, EDGE_THRESHOLD, EDGE_THRESHOLD, cv::BORDER_REFLECT_101 + cv::BORDER_ISOLATED);
			}
			else
			{
	...
	void ORBExtractorHLS(cv::Mat _image, cv::KeyPoint _keypoints[NUM_KEYPOINTS], cv::Mat _descriptors, float _scaleFactor) {
		...
		cv::Point patternT[512];
		for (int i = 0; i < 1024; i += 2) {
			patternT[i / 2] = cv::Point(bit_pattern_31_[i], bit_pattern_31_[i + 1]);
		}

		//for (int i = 0; i < pattern.size(); i++) {
		//	std::cout << pattern.at(i).x << "," << pattern.at(i).y<< " and " << patternT[i].x << "," << patternT[i].y << "\n";
		//}
		//for (int i = 0; i < 512; i++) {
		//	std::cout <<  patternT[i].x << "," << patternT[i].y << "\n";
		//}
		...
		Starting operator() functionality from here onwards
